...
3. Main シーン開始（オンラインゲーム）
   ...
   → コイントスで先攻後攻決定 → 初期手札（先攻3枚・後攻4枚）をドロー・ゲームループ開始
```
...
ホスト側                                         クライアント側
  ...
  │
  ├─ 先攻後攻を抽選 → 両デッキをシャッフル・初期手札(先攻3枚/後攻4枚)を決定・_opponentClientId 保存
...
- 初期手札枚数は先攻後攻を先に決めてから配る。先攻=3枚・後攻=4枚（`MulliganRule`）で、ホスト/クライアントそれぞれの枚数を `NGS_InitialState` の手札配列長と `opponentHandCount` で双方に伝える
