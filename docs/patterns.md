...
[MainPresenter.Phases.Resolution.cs](../Assets/Scripts/Main/MainPresenter.Phases.Resolution.cs) の `ResolveCharacterTriggeredEffectAsync`（OnEnter / OnAttack / OnDestroy / OnTurnStart 共通）の `switch` に `case CardEventType.Xxx:` を追加する。既存のイベント効果解決ヘルパー（`PlayDrawEffectAsync` / `ApplyDrawEffectAsync` / `PlayBanishCharEffectAsync` / `ApplyDamageAllEnemiesAsync` / `AddVictoryPoints` 等）を流用できる。現状は `Draw` / `DrawSkipNext` / `DrawNextTurnStart` / `BanishChar` / `DamageAllEnemies` / `DamageEnemy` / `SummonChar` / `SummonFromDeckByKeyword` / `CopyFieldChar` / `GainVPPerGreenGrave` / `HealAllAllies` / `NextCardCostFree` / `Bounce` / `BounceAll` / `ExtraTurn` / `BuffAttackByKeyword` / `BuffHpByKeyword` を実装済み（固定値の勝利点は `EventType` ではなく `VictoryPointBonus` 付帯値で付与する）。墓地など盤面状態から加点値を動的に算出する効果（`GainVPPerGreenGrave` は `CountGreenInGraveyard` → `GraveyardView.CountByAttribute`）は、`AddVictoryPoints` 等の既存ヘルパーに算出値を渡す形にする（墓地は同期済みのため決定的）。敵キャラを N 体選ぶ効果（`DamageEnemy` / `Bounce`）は対象選択を `ResolveEnemyCharTargetsAsync`（プレイヤー選択／CPU 自動／オンラインはインデックス同期。トースト文言は引数）で共用する。2つの数値が必要な効果（SummonChar の「ID」と「体数」など）は `EventValue2` / `EffectValue2` を使う（未使用の効果は 0）。コスト支払いに作用する効果（`NextCardCostFree`）はプレイヤーごとの永続フラグを持ち、`PayHandCostAsync` 側でコスト0化・消費する（[event.md](event.md)「効果ごとの注意点」参照）。ターン進行に作用する効果（`ExtraTurn`）はアクティブプレイヤーのフラグ（`_extraTurnPending`）を立て、`RunTurnAsync` 末尾で `GameModel.RepeatTurn()` を呼んで同じプレイヤーがもう一度ターンを行う（オンラインは Pass 時の相手ドロー待ち登録をスキップして lockstep を維持。[event.md](event.md)「効果ごとの注意点」参照）。フィールドのキャラのステータスを永続変化させる効果（`BuffAttackByKeyword` / `BuffHpByKeyword`）は `CardView` に実行時バフ状態（`_attackBuff` / `_hpBuff` → `CurrentAttack` / `MaxHp`）を持たせ、`ApplyBuffByKeywordAsync` で発動側の自フィールドから条件に合うキャラ（同じ `CardData.Keyword`・自分以外）を集めて `BuffAttackAsync` / `BuffHpAsync` で加算する。**攻撃力を実行時に変える効果を追加するときは、戦闘ダメージ・CPU判断・対象選択の攻撃力参照を `Data.Attack` ではなく `CardView.CurrentAttack` に通すこと**（バフが反映されなくなるため）。任意の文字列で対象を絞る「特徴（キーワード）」は `CardData.Keyword`（マスターは `CardKeywordSO`）で管理し、文字列一致で判定するため実行時の追加ロード・同期は不要（[event.md](event.md)「特徴（キーワード）」参照）。
