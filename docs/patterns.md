...
- 同期処理のみ（BattleEndMill のフィールドセット相当）なら `break` で完結
...
デッキからカードが墓地に送られたとき（戦闘ダメージ）に自動で効果が発動するカード。手札コスト払いで墓地に送られた場合は発動しない（デッキ経由ではないため）。
...
③ 発動タイミングのフェーズ処理末尾でフィールドを参照する（例: メインフェーズの攻撃終了後、`ResetTurnState()` の後）:
