...
- **効果適用前に演出**（Draw / BanishChar / GainVictoryPoints）: `await ApplyEventEffectAsync(...)` の直前に `else if` で演出を先に呼ぶ。Draw はドロー前の予告、BanishChar は対象キャラ上に「BANISH!」ラベル+パーティクル、GainVictoryPoints は MedalIcon フロートを表示してから効果適用。
