...
ID は SO の `OnValidate` で**自動採番**される（`CardIdAutoAssigner`。CharacterCardSO = `C###`、EventCardSO = `E###`）。空の ID と重複した ID のみ次の空き番号が割り当てられ、既存の一意な ID は変更されない。新しい SO を追加する場合も同じ要領で `OnValidate` + `CardIdAutoAssigner.AssignIds(_cards, "X")` を実装する。
