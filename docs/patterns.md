...
> **キャラ単位の実行時状態を増やす効果（バフ・能力付与など）**: 攻撃力/HP バフ（`BuffAttack/Hp`）やキーワード能力付与（`GrantKeyword`）のように、キャラ個体へ永続的に乗る状態は [CardView.cs](../Assets/Scripts/Main/Card/CardView.cs) に持たせる。①フィールド（例: `_attackBuff` / `_grantedGuardian`）を追加し、②**SO 固有値と実行時付与を合成した参照プロパティ**（`CurrentAttack` / `MaxHp` / `HasGuardian` 等）を公開して、戦闘判定や表示はこのプロパティ経由に統一する（SO の生フィールドを直接読まない）。③`CopyRuntimeStateFrom` にも複製を追加する（`CopyFieldChar` でコピー元の状態を引き継ぐため）。例として `GrantKeyword` は守護/速攻/飛行/防人/強襲/デッキ攻撃×/必殺の付与フラグと `HasGuardian` / `HasHaste` / `HasFlying` / `HasSakimori` / `HasAssault` / `HasNoDeckAttack` / `HasDeadly` プロパティを追加し、`MainPresenter.Phases.Main` の `IsGuardian` 等の攻撃ルール判定を `card.Has*` 参照へ切り替えた。付与キーワードと値2の対応は [GrantableKeyword.cs](../Assets/Scripts/Main/Card/GrantableKeyword.cs)。新キーワードを足すときは GrantableKeyword enum・CharacterCardData フラグ・CardView の `Has*`/granted/Reset/Copy・RefreshKeywordIcons・GrantKeywordAsync・Card.uxml/uss のアイコン・CardDetail.uss/CardDetailModal の説明・CardEditorWindow のチェックボックス・GrantKeywordHandler の値2リスト＋**そのキーワードが作用する箇所**をまとめて触る。作用する箇所はキーワードの性質で異なり、**攻撃対象の選択ルール**を変える能力（強襲・守護・防人・飛行など）は `CanAttackChar` / `CanAttackDeck`、**戦闘ダメージの解決**を変える能力（必殺＝ダメージ計算せず対象を破壊）は `ExecuteAttackAsync`（[MainPresenter.Phases.Main.Attack.cs](../Assets/Scripts/Main/MainPresenter.Phases.Main.Attack.cs)）を触る。CPU が攻撃力0でも有効に使えるキーワード（必殺）は `MainPresenter.Phases.Main` の CPU 攻撃者候補・対象選択も合わせて見直す。
>
> **「次の相手ターンだけ」など期限つきの実行時状態**（`FreezeEnemyChars` の凍結）も同じく `CardView` に持たせる：単純な bool（`_frozen` ＋ `IsFrozen` / `SetFrozen`）を追加し、攻撃可否の単一ゲート `MainPresenter.Phases.Main` の `CanCharAttack` で `if (card.IsFrozen) return false;` を返す（通常攻撃・デッキ攻撃の両方を止める）。`SetFrozen` は表示（`frozen-char` 枠＋`frozen-overlay`/`frozen-icon` の氷結オーバーレイ）も一緒に切り替える。**期限の解除は `RunTurnAsync` のターン終了処理にフックを足す**（`ClearFrozenChars` を手番側の場へ呼ぶ＝1ターンで消費）。`CopyRuntimeStateFrom`／`ResetRuntimeState` への複製・解除も忘れず追加する。盤面とターン進行から決定的に解決されるためオンライン・CPU でも追加同期は不要（対象選択は既存の `NGS_DamageTarget` を流用）。
...
- 敵キャラを N 体選ぶ効果（`DamageEnemyHandler` / `BounceHandler` / `BanishCharHandler` / `DebuffAttackHandler`）は対象選択を `ResolveEnemyCharTargetsAsync`（プレイヤー選択／CPU 自動／オンラインはインデックス同期。トースト文言は引数）で共用する MainPresenter 側メソッドを呼ぶ。`DebuffAttackHandler` は値1=対象数（0=敵全員）・値2=減少量で `ApplyDebuffSelectedEnemiesAsync` を呼ぶ。
...
- デッキ／墓地などの**非公開ゾーンからカードを選んで動かす効果**（`SummonFromDeckByKeywordHandler` / `AddToHandFromDeckByKeywordHandler` / `SummonFromGraveHandler` / `AddToHandFromGraveHandler`）は、対象ゾーンの `GetCardDataSnapshot()`（インデックス付きコピー）で候補を抽出 → インデックスで `RemoveCardAt`（複数取り除くときはインデックスを詰めるため**降順**で）→ 配置・手札追加へ運ぶ。`AddToHandFromGraveHandler` は墓地版で特徴フィルタを持たず墓地の全カード（キャラ・イベント問わず）を候補にして手札へ加える（場に出さないため OnEnter は発動しない・手札上限超過分は墓地へバーン）。選択はプレイヤー＝ピッカー（単数は `WaitForPlayerDeckCardSelectionAsync`、複数選択は共通の **`WaitForPlayerCardsPickAsync`**＝タイトル/サブタイトル/追加CSSクラスを差し替えてデッキ・墓地で共用）／CPU＝高コスト順／オンライン＝選んだインデックス配列を `NGS_DamageTarget` チャネルで同期（ゾーンの並び順は両クライアントで同期済みのため決定的）。場へ出す系はフィールドの空きスロット数（`FieldView.MaxCharacters` − 現在キャラ数）で体数の上限を打ち、ゾーンから抜いたのに置けず消える事故を防ぐ。`GraveyardView` も `DeckView` と同じ `GetCardDataSnapshot` / `RemoveCardAt` を備える。
- デッキにダメージを与える（デッキ→墓地へミルする）効果（`DamageEnemyDeckHandler` / `DamageOwnDeckHandler` / `DamageBothDecksHandler`）は `p.MillDeckAsync(deckOwnerIsLocal, count, ct)` を呼ぶ。デッキ攻撃と共通の building-block で、ミル・ダメージトリガー（`TriggerOnGrave`）起動・オーバーリミット判定（空デッキからさらにミルしようとした瞬間に持ち主が敗北）・オーバーリミット告知をまとめて行う。発動側から見た相手デッキは `deckOwnerIsLocal: !inv.IsLocal`、自分のデッキは `inv.IsLocal`。デッキ順は同期済みのため追加同期不要。
- **デッキにカードを追加する**効果（`JamEnemyDeckHandler` ＝お邪魔トークンを相手デッキに混ぜる）は、ID（`E{値2}`）から `CardDatabase.TryGet` でトークンを解決して N 枚分のリストを作り、`DeckView.AddCardsAndShuffle` で追加＋シャッフルする（`p.ApplyJamEnemyDeckAsync`）。**シャッフルは乱数を使うため `Recover` と同じ方式でオンライン同期する**：発動側（`!_isOnline || isLocal`）がシャッフル後のデッキ ID 列を `NGS_JamDeck` で送り、ミラー側は受信ハンドラをアニメ前に登録（ロスト防止）して `DeckView.Rebuild` で組み直す。トークンは `CardDatabase._dict` 登録済み（「ゲームで使用」ON）なら `BuildDeck` で解決できる。デッキ順を変える効果（`Recover` / `JamEnemyDeck`）はこの送受信パターンを踏襲する。
- **複数の building-block を1効果に合成する**例：`DamageDeckRecoverByColorCharsHandler` は盤面カウント（`p.CountCharsOnFieldByAttribute` ＝ `GainVPPerGreenGrave` の動的カウントと同型）→ ミル（`MillDeckAsync`）→ 墓地回収（`ApplyRecoverEffectAsync`）を `p.ApplyColorCharDeckDamageAndRecoverAsync` 1メソッドに並べる。**枚数 N は一度だけ算出して各処理で共有**し、ミルで持ち主がデッキ切れ敗北したら（`_isGameOver`）後続をスキップする。各 building-block が自前でオンライン同期するため合成側に追加同期は不要。なお属性を**値で指定する**ときは `CardAttributeNames.FromNumber`（白1/青2/緑3/黄4/赤5/黒6/紫7。0=全属性扱い・範囲外は null）で実行時に解決する（`CardIdAutoAssigner.AttributeNumber` は Editor 専用で同じ並び）。`RecoverByColorCharsHandler` は同じ盤面カウント → `ApplyRecoverEffectAsync` の回収だけを `p.ApplyRecoverByCharCountAsync` に並べた**ミルなしの回収のみ版**で、building-block の組み合わせを変えるだけで別効果を作れる好例。
...
- フィールドのキャラのステータスを永続変化させる効果（`BuffAttackByKeywordHandler` / `BuffHpByKeywordHandler` / `AtkBoostHandler` / `HpBoostHandler` / `DebuffAttackHandler`）は `CardView` の実行時バフ（`_attackBuff` / `_hpBuff` → `CurrentAttack` / `MaxHp`）を加減算する。攻撃力を上げる効果（`BuffAttackByKeywordHandler` / `AtkBoostHandler` → `CardView.BuffAttackAsync`）は `_attackBuff` を加算し、オレンジパルス＋「攻撃力 +N」フローティングラベル（`attack-buff-label`）を出す（`isAttack` のときのみ。HP バフ側は出さない）。下げる効果（`DebuffAttackHandler` → `CardView.DebuffAttackAsync`）は `_attackBuff` を**減算しつつ `CurrentAttack` が0未満にならないようクランプ**し、青パルス＋「攻撃ダウンN」フローティングラベル（`debuff-attack-label`）を出す。**攻撃力を実行時に変える効果を足すときは、戦闘ダメージ・CPU判断・対象選択の攻撃力参照を `Data.Attack` ではなく `CardView.CurrentAttack` に通すこと**。
...
- **判定箇所**: [CardData.cs](../Assets/Scripts/Main/Card/CardData.cs) の `public virtual int CostPaymentValue(CardAttribute payingForAttribute) => 1;` を、[CharacterCardData.cs](../Assets/Scripts/Main/Card/CharacterCardData.cs) / [EventCardData.cs](../Assets/Scripts/Main/Card/EventCardData.cs) で `override` してカードの種別/値から導出する（CostBoost なら、自属性が `payingForAttribute` と一致するとき `Max(1, 値)`、それ以外は 1。白も一般属性扱いで、白 CostBoost は白のコストのみ倍化）。**「コスト素材にできない」フラグ（`CardData._cannotBeUsedAsCost`・お邪魔トークン用）が立っていれば最優先で 0 を返す**（両 override の先頭で判定）。0 を返すカードはコスト選択 UI でも選べない（`HandleCostCardClick` でガード）。引いても出せない高コストカードと併用すると、コストにも回せない「死に札」になる。
