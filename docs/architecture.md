...
DeckModel   シングルトン。シーンをまたいでデッキ内容を保持
  ...
  IsValid       IsReady かつ IsCostOver でないとき true（デッキ有効判定）
...
CharacterCardSO / EventCardSO   各カード種別の ScriptableObject（インスペクター編集用）
                  ID は OnValidate でリスト順に自動採番（CardIdAutoAssigner・エディタ専用。CharacterCardSO=C### / EventCardSO=E###。先頭要素が X001 で、追加・削除・並び替えのたびに全 ID を振り直す）
...
CardView          VisualElement サブクラス。Card.uxml をクローンしてデータをバインド
                  ...
                  カード情報は左上に縦一列表示：カード名 → コスト → 属性 → （キャラ）攻撃力 → HP
...
LifeHeartsView       VisualElement サブクラス。ハート勝利条件のライフ表示（HeartIcon.png ×3）
                     ...
                     （コンテナのアンカー方向と破壊時の詰まり方向を一致させ、パーティクル位置とギャップを揃えるため）
...
```
BattleArea
  ├── OpponentFieldArea（上半分、gap: 32px で分離）── FieldView（相手フィールド）＋ LifeHeartsView（右上・相手ライフ）
  └── PlayerFieldArea  （下半分）── FieldView（自分フィールド）＋ LifeHeartsView（左下・自分ライフ）
```
...
BattleArea              （top: 100px〜bottom: 144px）
  ├── OpponentFieldArea（上半分）── FieldView（相手フィールド）＋ LifeHeartsView（右上）
  └── PlayerFieldArea  （下半分）── FieldView（自分フィールド）＋ LifeHeartsView（左下）
DeckArea                （右下・60% スケール）
GraveyardArea           （左下）
...
4. 相手・自分ともに `HandView`・`DeckView`・`FieldView`・`GraveyardView`・`LifeHeartsView` を配置
