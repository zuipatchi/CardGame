...
- `SoundStore` が BGM・SE クリップをロード（`MaouOrchestra`, `MainBGM`, `EnterSE`, `Enter2SE`, `Cancel1SE`）
...
EventType         enum（EffectType.cs）。
                  ...
                  DeckMill（相手デッキ上から EventValue 枚を即時墓地へ）
...
RunBattlePhaseAsync
  → "FIGHT" 告知
  → ATK = キャラカードの Attack + AtkBoost（属性倍率なし）。スロット空の場合は ATK = 0
  → PlaySingleSideAtkCounterAsync (先攻): 先攻側に 0→ATK のカウントアップ表示、**キャラスロット**に DEF アイコン＆DEF値をフェードイン表示
  → PlayAttackIconAsync (先攻) or "NO ATTACK": ATKカウンターオーバーレイが後攻キャラスロットへ飛翔 → PlayDeckDamageAsync で先攻ダメージ適用（後攻デッキが 0 なら OnGameEnd(isLocalFirst) で先攻勝利）
  → PlaySingleSideAtkCounterAsync (後攻): 後攻側に 0→ATK のカウントアップ表示
  → PlayAttackIconAsync (後攻) or "NO ATTACK": ATKカウンターオーバーレイが先攻キャラスロットへ飛翔 → PlayDeckDamageAsync で後攻ダメージ適用（先攻デッキが 0 なら OnGameEnd(!isLocalFirst) で後攻勝利）
```
