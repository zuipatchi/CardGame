...
CommonLifetimeScope   全シーン共通のシングルトンを登録
  ├── DeckModel
  ├── DeckRepository
  ├── StarterDeckSeeder   初回起動時にスロット1へ初期デッキをシード（IStartable）
  ├── GameSessionModel
...
- **対処（必須）**：常駐する `Common` シーンに `EventSystem` + `InputSystemUIInputModule` を1つ追加し、Pointer Behavior を **Single Unified Pointer**（マウスと各タッチを単一ポインターに統合）にする。これで UI Toolkit のドラッグがマウスと同じ挙動で安定する。`Hierarchy → UI → Event System` で追加し、`InputSystemUIInputModule` の Pointer Behavior を Single Unified Pointer に変更する（Common シーンには設定済み）。
...
### 初期デッキの自動生成（StarterDeckSeeder）

初めてゲームを起動したプレイヤーがデッキ構築をせずにすぐ対戦できるよう、`StarterDeckSeeder`（`CommonLifetimeScope` に `IStartable` で登録）が起動時にスロット1（index 0）へ初期デッキをシードする。中身は Addressables キー `Deck/StarterDeck`（`StarterDeckSO`）が持つカードID配列で、現状は `CpuDeck.asset` と同じ30枚（赤単）。`StarterDeckSO` は Common 側に置く（Common は Main を参照できないため `CpuDeckSO` は流用せず同形の SO を新設）。

シードは初回のみ。`DeckRepository` の `StarterDeckSeeded` フラグ（PlayerPrefs）で二重生成を防ぎ、**既に何らかのデッキを持つ既存プレイヤーには上書きせずフラグだけ立てる**（`HasAnyDeck()` 判定）。旧単一デッキがスロット1へ移行済みのプレイヤーも `HasAnyDeck()` が真になるため安全。Addressables ロードに失敗した場合はフラグを立てず次回起動で再試行する。保存は `SeedSlotWithIds`（カードIDのみ。**コストは対戦・表示のどちらでも使われず `CardData` から再解決されるため 0 で保存**する。デッキ分析モーダルのマナカーブも `CardDatabase` 解決に統一済み）。
...
CardView          VisualElement サブクラス。Card.uxml をクローンしてデータをバインド
                  ...
                  アイコン群（game-card__icons）は縦並びだが、表示中のアイコンが4つを超えると flex-wrap で右隣の2列目へ折り返す（確定値 height 168px ＝ 40px×4 に余裕。display:none のアイコンは数えない）。カードの高さは伸びない。折り返し数は端末・配置によらず常に縦4つ（max-height だと Yoga の column flex-wrap が非決定的になり実機で個数がブレるため、確定値 height ＋ flex-shrink:0 で固定している）
...
9. VS 告知の後、配牌前に `InitializeFirstTurnAsync` でコイントス演出を行い先攻後攻を確定する（手札枚数が先攻3枚・後攻4枚で変わるため配牌前に決める）
10. 先攻3枚・後攻4枚（`MulliganRule`）を 0.12 秒ずつずらして `AddCardAnimatedAsync` で配牌 → マリガンチェック
...
InitializeFirstTurnAsync  （ゲーム開始時に1度だけ・配牌前に実行）
  ...
  （補正ドローなし。先攻有利は手札枚数差（先攻3/後攻4）と両プレイヤーの初手ドローなしで補正する）
...
- **カードの見方（`CardReading`）**: 戦闘しない練習。盤面プリセットなしで、手札・デッキは全てキーワード能力持ちの実在カード（守護/飛行/速攻/防人/強襲/必殺）で構成しトークンを入れない。step0=手札アイコンを解説し先頭カード（C1005）をハイライト→手札カードを開いて閉じると step2=カード詳細の読み方を解説＋END をハイライト→END（パス）で `CompleteTutorial`。step0/1 のうちに END を押しても完了せず次の自分の番で解説へ戻る。
...
- **オーバーリミット（`OverLimitWin`）**: 「デッキを0枚にしただけでは勝てない＝オーバーリミット状態」を体験させる2ターン構成。`DeckOutWin` と同じく攻撃役（C1001・攻2）を1体プリセットするが、相手デッキを残り2枚にする（CPU 6枚＝後攻手札4＋デッキ2）。1ターン目に攻2でちょうど0枚にしても勝てず（`MillDeckAsync` のミル直前 `CheckDeckOutWin` は空デッキを「さらに削ろうとした」次のミルで初めて敗北させる）、`TutorialOnLocalActionResolved` がデッキ攻撃解決時にオーバーリミット状態を説明して END をハイライト。次の自分の番（step1）にもう一度デッキ攻撃すると空デッキへのミルで `CheckDeckOutWin` が成立しデッキ切れ勝利。
- **クリア演出**: 勝ち方系（`DeckOutWin` / `OverLimitWin` / `VictoryPointsWin`）は通常の勝利条件（`winReason` 付き）で決着し「デッキ切れ勝利／勝利点勝利」を表示。基本・キーワード系は `CompleteTutorial` が `_tutorialCompleted=true` を立てて `OnGameEnd(playerWins:true)` を呼び、`PlayGameEndAsync` が「YOU WIN」ではなく **「チュートリアル完了」** を表示する。チュートリアルは再戦（同じ台本の再ロード）に意味がないため、`PlayGameEndAsync` は `_isTutorial` のとき再戦ボタンと使用デッキ変更ボタン（固定デッキのため）を隠し「ホームに戻る」のみ表示する。
...
- **キーワード体験に使う実在カード**: 守護=C1005 / 速攻=C1009 / 飛行=C5003 / 防人=C3007（守護兼）/ 強襲・デッキ攻撃×=C3006（速攻兼）/ 必殺=C7006。単一キーワードの専用カードが無いキーワードは複数キーワードを持つ既存カードで代用する。必殺チュートリアルは攻撃役に必殺の C7006（攻1）、的に高HP（HP8）の C2008（タップ済み）をプリセットし、攻撃力1でも一撃で大型キャラを破壊する体験をさせる。
