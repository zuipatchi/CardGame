...
FieldView         VisualElement サブクラス。画面中央の横長フィールドエリア
                  最大 5 枚を中央寄せで表示。配置済みカードはドラッグ不可
...
4. 先頭 5 枚を `HandView`（画面下部中央）、残りを `DeckView`（右下）、`FieldView`（画面中央）として配置
5. `HandView.OnCardDropped → FieldView.TryPlace` でドロップ配置を接続
