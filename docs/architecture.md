...
Assets/AddressableAssets/
  ...
  ├── Icon/        HeartIcon.png（コスト・デッキ枚数バッジ用）、AttackIcon.png（攻撃力バッジ用）、GraveIcon.png（墓地枚数バッジ用）
  ├── Image/       CardBack.png（カード裏面画像）、NamePlate.png（カード名プレート背景）、Card*.png（カードイラスト）、BattleField.png（盤面背景）、OKButtn.png（OKボタン画像）、ReturnButtn.png（戻るボタン画像）
  ├── Modal/       Modal.uxml
  └── Sound/       AudioClip
```
...
5. `HandView.OnCardDropped → playerFieldView.TryPlace` でドロップ配置を接続
   ...
   - ステージ中・相手ターン中・アニメーション中は新たなドロップをブロック
6. Resolve オーバーレイ（`resolve-overlay` / `resolve-label`）とターン告知オーバーレイ（`turn-announcement-overlay` / `turn-announcement-label`）を生成
7. `GameModel.OnResolveAsync = HandleResolveAsync`（非同期アニメーション付き攻撃処理）
   `GameModel.OnTurnChanged += OnTurnChanged`（CPU ターン開始トリガー）
   `GameModel.OnResolvePhaseStartAsync`（Resolve テキスト演出）
   `GameModel.OnTurnStartAsync`（YOUR TURN / ENEMY TURN 演出）を接続
8. OK/戻るボタンに `ConfirmAction` / `CancelAction` を接続
9. `UniTask.NextFrame` でレイアウト確定を待つ
10. 自分・相手ともに 5 枚を 0.12 秒ずつずらして `AddCardAnimatedAsync` を並走
    - 自分：デッキ位置から飛翔→フリップ（表向き）
    - 相手：相手デッキ位置から飛翔→裏向きのまま手札へ収まる
11. 相手手札カードを `_cpuCards` HashSet に登録（撃破時に相手フィールド・墓地を判定するため）
12. `HandleTurnStartAsync(isLocalTurn: true)` でゲーム開始時の "YOUR TURN" 演出を表示
    - 演出と同時に `DeckView.DrawTop()` でデッキ上部から 1 枚ドローし `AddCardAnimatedAsync` でアニメーション
...
### CPU ターン処理（MainPresenter + CpuAgent）

```
OnTurnChanged(isLocalTurn: false)
  → ProcessCpuTurnAsync().Forget()
    → CpuAgent.TakeTurnAsync() await
      → 1秒待機
      → TryDecide() で行動決定
      → 矢印を _pendingArrows に登録
      → GameModel.DoAction() await
```

- CPU ターン中は `IsLocalTurn = false` のためプレイヤーはアクション不可
- CPU のカードを表向きにするため `FlipAsync` を `DoAction` 前に呼ぶ
