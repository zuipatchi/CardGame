...
CardData              抽象基底クラス。id / name / cost / Attack / Defense / Speed / Hp / image(Sprite)
  ...
  EventCardData       イベントカード。EventType（None/AtkBoost/DefBoost/Draw/Negate/BanishChar/Recover/Switch/CharDamage/Evolve/Poison/DeckMill/BattleEndMill）と EventValue を保持
...
TurnPhase         enum。CharacterSet / Draw / PreBattle2 / Battle
```
...
FieldView         VisualElement サブクラス。横長フィールドエリア（最大 5 枚、中央寄せ）
                  ...
                  PlaceCard(CardView) で配置（ドラッグ不可に設定）、RemoveCard(CardView) で除去
...
BattleArea              （top: 100px〜bottom: 144px）
  ├── OpponentFieldArea（上半分）── FieldView（相手フィールド）
  └── PlayerFieldArea  （下半分）── FieldView（自分フィールド）
DeckArea                （右下・60% スケール）
GraveyardArea           （左下）
...
4. 相手・自分ともに `HandView`・`DeckView`・`FieldView`・`GraveyardView` を配置
...
```
RunCharacterSetPhaseAsync  （毎ターン DrawPhase の直後に実行）
  → "キャラセットフェーズ" 告知
  → 両プレイヤーが同時操作（1ターン1枚まで、CanPlayerSetChar / CanOpponentSetChar フラグで管理）
      CPU戦:    UniTask.WhenAll で PlayerCharSetLocalAsync と CpuCharSetAsync を並列実行
      オンライン: receiveTask（WaitForOpponentCharSetAsync）をホットタスクとして並列起動
                  自分の入力完了後に SendCharSetAction で送信 → await receiveTask で相手受信完了を待機
  → Resolve アニメーション → 新たに置かれたカードを表向き（IsFaceDown の場合のみ FlipAsync）→ コスト払い
```
...
先攻後攻決定（RunTurnAsync 内）
  → DetermineFirstMoverByField() でフィールド最高速キャラの Speed を比較
      Speed 差あり: 高い方が先攻
      Speed 同値:   攻撃優先権保持者が先攻（行使後は相手に移る）
...
RunResolutionPhaseAsync
  ...
  → イベントカード効果を効果種別に応じて演出＋適用
     ...
     BanishChar: 先に PlayBanishCharEffectAsync で対象キャラ上に「BANISH!」フローティングラベル（赤・上昇フェード）+ パーティクルを再生。ApplyEventEffectAsync 内で FlyCardToDestAsync によりキャラカードを墓地へ飛翔させてから AddCard
...
RunBattlePhaseAsync
  → "FIGHT" 告知
  → 攻撃選択: プレイヤーはフィールドのキャラをドラッグして相手キャラへ矢印を引く（Pass ボタンでパス可）
              相手キャラなし → 最高ATKキャラでデッキ直撃（target=null）
              自キャラなし  → 自動パス
    CPU: CpuChooseBattleAttack() で最高ATKキャラを選択
  → 速さ比較で先攻後攻を決定（同値は攻撃優先権）
  → ATK = attacker.Attack + AtkBoost。対象 DEF = target.Defense + DefBoost（target=null なら DEF=0）
  → PlaySingleSideAtkCounterAsync (先攻): 先攻側に 0→ATK のカウントアップ表示
  → PlayAttackIconAsync (先攻) or "NO ATTACK": ATKカウンターオーバーレイが後攻対象へ飛翔 → ダメージ適用 → キャラ破壊判定
  → PlaySingleSideAtkCounterAsync (後攻): 後攻側に 0→ATK のカウントアップ表示
  → PlayAttackIconAsync (後攻) or "NO ATTACK": 同様
```
...
入力状態は `StagedInput` ネストクラス（`Tcs` + `Card` フィールド）で管理。フェーズごとに `_charSetInput` / `_prepInput` の2インスタンスを保持する。  
戦闘入力専用に `_battleAttackTcs: UniTaskCompletionSource<(CardView attacker, CardView target)>` と `_fieldCharSelectionTcs: UniTaskCompletionSource<CardView>` も保持する。

**キャラセットフェーズ（WaitForPlayerCharSetInputAsync）:**
```
_charSetInput.Tcs を作成して ShowActionButtons()
  → キャラカードをフィールドへドロップ → _charSetInput.Card にセット → 即座に FlipAsync（裏向き）→ OK/戻るボタン表示
...
**戦闘フェーズ（WaitForPlayerBattleAttackAsync）:**
```
_battleAttackTcs を作成し、自軍キャラに AttackArrowManipulator を取り付け
  → キャラ 0 体       → (null, null) を即時セット（自動パス）
  → 相手キャラ 0 体   → 最高ATKキャラ + target=null を即時セット（デッキ直撃）
  → ドラッグで矢印を相手キャラへ → AttackArrowManipulator が (attacker, target) をセット
  → パスボタン        → (null, null) をセット
finally: アタッカーから AttackArrowManipulator を除去
```
...
CpuAgent.ChooseBattleAttack(ownChars, opponentChars)        戦闘フェーズ：最高ATKキャラで最低ATK相手キャラを攻撃（(attackerIdx, targetIdx) を返す）
```

- キャラセットフェーズ: `ChooseCharacterSetCardIndex` でキャラカードを選択して `FieldView` へ裏向きで飛翔配置
- 戦闘前2フェーズ: `ChooseEventCardIndex` でイベントカードを選択し `FieldView` へ飛翔・Ready 化（`RunCpuPreBattle2SubTurnAsync`）
- 戦闘フェーズ: `ChooseBattleAttack` で攻撃者と対象を決定（`CpuChooseBattleAttack()` ラッパー経由）
...
GameModel         フェーズ遷移と戦闘前2フェーズの入力状態を管理
  ...
  CanPlayerSetChar         今ターンにプレイヤーがキャラをセットできるか（1ターン1枚制限）
  CanOpponentSetChar       今ターンに相手がキャラをセットできるか（1ターン1枚制限）
  ...
  RecordPlayerCharSet()    CanPlayerSetChar を false に（今ターンの1枚セット済みを記録）
  RecordOpponentCharSet()  CanOpponentSetChar を false に
  EndTurn()                キューをクリア・Phase → Draw・IsLocalTurn を反転・CharSet フラグをリセット
```
