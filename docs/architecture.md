...
RunMainPhaseAsync  （手番プレイヤーは EndButton／Pass を出すまで制限なく行動を繰り返すループ）
  ...
  → キャラへの攻撃対象は**タップ状態のキャラのみ**（攻撃したキャラはタップ→自分のターン開始時にアンタップ／守護・防人は毎ターン終了時に自動タップ＝AutoTapGuardiansAndSakimoriAsync）。判定は CanAttackChar が IsTapped を含めて中央化。ただし**強襲（Assault）**を持つ攻撃者はタップ要件を無視しアンタップのキャラも狙える。デッキ攻撃はタップ制限の対象外
