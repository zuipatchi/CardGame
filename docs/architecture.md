...
DeckModel   シングルトン。シーンをまたいでデッキ内容を保持
  ...
  IsOver        TotalCost > TargetCost (30) のとき true（コストオーバー判定）
...
10. オフラインのみ：相手手札カードを `_cpuCards` HashSet に登録。オンライン・オフライン共通で `RunGameAsync(isLocalFirst, ct)` でゲームループ開始（先攻後攻はオンラインは `OnlineInitialState.IsLocalFirst`、オフラインはランダム）
...
RunCharacterSetPhaseAsync  （ゲーム開始時1回のみ）
  → "キャラセットフェーズ" 告知
  → 先攻・後攻の順に処理
      オフライン: 自分 → WaitForPlayerCharSetInputAsync / CPU → CpuAgent で選択
      オンライン: 自分のターン → WaitForPlayerCharSetInputAsync → SendCharSetAction で送信
                  相手のターン → WaitForOpponentCharSetAsync で受信 → PlayOpponentCharSetOnlineAsync でアニメーション
