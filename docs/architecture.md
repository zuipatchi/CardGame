...
HandView          VisualElement サブクラス。手札を扇状に表示（75% スケール）
                  ...
                  ホバー時に DOTween で 1.2 倍スケールアップ＋BringToFront() で最前面表示
...
4. 先頭 5 枚を `HandView`（画面下部中央）、残りを `DeckView`（右下）として配置
5. `OpponentFieldArea` と `PlayerFieldArea` にそれぞれ `FieldView` を配置
6. `HandView.OnCardDropped → playerFieldView.TryPlace` でドロップ配置を接続
