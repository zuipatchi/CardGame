...
DeckModel   シングルトン。シーンをまたいでデッキ内容を保持
  ...
  Count         現在の枚数
  TotalCost     現在のコスト合計
  IsReady       TotalCost == TargetCost (70) のとき true
  IsOver        TotalCost > TargetCost (70) のとき true（コストオーバー判定）
...
CardData              抽象基底クラス。id / name / cost / Attack / Defense / Speed / Hp / image(Sprite)
  ...
  EventCardData       イベントカード。EventType（None/AtkBoost/DefBoost/Draw/Negate/BanishChar/Recover/Switch/CharDamage/Evolve）と EventValue を保持
