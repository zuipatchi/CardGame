...
|---|---|---|
| None | 効果なし | － |
| Draw | 発動プレイヤーが指定枚数デッキ上から手札に加える。**オーバーリミット**：1枚引く直前にデッキが空（0枚）なら、その瞬間に発動プレイヤーがデッキ切れで敗北する（2枚引く効果でデッキ1枚なら、1枚引いてデッキ0→2枚目を引こうとして敗北。[rules.md](rules.md)「勝敗条件」）。手札が上限（8枚）の場合は引いたカードを墓地へ送る（バーン。[rules.md](rules.md)「手札の上限」）。**値2でドロー方式を切り替える**（0=通常 / 1=コインドロー / 2=サイコロドロー。下記） | 値1=ドロー枚数 / 値2=0通常/1コイン/2サイコロ |
| Draw（コインドロー：値2=1） | コインを振り、**表が出るたびにカードを1枚引き、裏が出たら終了**する（乱数で判定。引く枚数は0枚もありうる）。1トスごとにカード付近でコイン演出を再生する（回転中は Coin SE を鳴らし回転終了で停止）。値1は無視。**引く枚数はデッキ残り枚数で上限を打つ**ため、空デッキから引いて強制敗北することはない（最後の1枚を引いてデッキが0になったときのリミットブレイク告知は通常 Draw と同じ）。手札上限バーンも通常 Draw と同じ。**オンライン**：発動側が振った「表の回数」だけを相手へ送信し（`NGS_CoinDraw`）、ミラー側は同期済みデッキの上から同じ枚数を引く（デッキ順が同期しているため同じカードになる） | 値1=無視 / 値2=1 |
| Draw（サイコロドロー：値2=2） | **6面サイコロを振り、出た目（1〜6）の数だけドローする**（乱数で判定）。カード付近で DiceIcon を転がして出目に着地する演出を再生する。値1は無視。引く枚数は確定値なので**通常 Draw と同じ扱い**（出目がデッキ残り枚数を超えればオーバーリミット敗北あり）。手札上限バーンも通常 Draw と同じ。**オンライン**：発動側が振った出目だけを相手へ送信し（`NGS_DiceDraw`）、ミラー側は同期済みデッキの上から同じ枚数を引く | 値1=無視 / 値2=2 |
...
> SO のインスペクターで直接編集するほか、メニュー **`Card → カードエディタ`**（[CardEditorWindow.cs](../Assets/Scripts/Editor/CardEditorWindow.cs)）で全属性のカードを横断して検索・編集・追加・削除できる。一覧はツールバーの「並び」で **ID順 / コスト順**に切り替えられる（コスト順表示時は各行に `[コスト]` を前置）。EventType に応じて値1/値2 の意味がラベル・ヒント表示され、画像は ObjectField で割り当てられる。採番ルールを変えた後は同ウィンドウの「ID再採番」で全カードを一括で振り直せる（旧 ID を参照する SummonChar 値・保存デッキは別途修正が必要）。効果テキスト（`Description`）はメニュー **`Card → 効果テキストを一括生成`** で全カード分をまとめて下記ルールから生成できる（手動編集したテキストも上書きされる。カードエディタが開いていればその表示も更新される）。
...
|---|---|
| None | （空文字） |
| Draw | カードをn枚引く（値2=1「コインを振り、表が出るたびにカードを1枚引く（裏が出たら終了）」／値2=2「サイコロを振り、出た目の数だけカードを引く」） |
