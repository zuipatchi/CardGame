...
| Deadly | **必殺**。ON にすると、このキャラが**相手キャラを攻撃した時、ダメージ計算を行わず対象を破壊する**（体力に関係なく無条件破壊・攻撃力0でも破壊）。回復・反撃を行う `OnAttacked` で生き残ることはできない（無条件破壊）。被攻撃→破壊の流れのため対象の `OnAttacked` / `OnDestroy`・攻撃側の `OnKill` は通常どおり発動する。相手デッキへの攻撃（デッキ攻撃）には影響しない（キャラ対象がないため）。`EffectType` とは独立したフラグで、攻撃のみに作用する。カードと詳細モーダルに SlayerIcon を表示（詳細は [rules.md](rules.md)「攻撃の詳細」） |
| Description | 効果説明（詳細モーダル表示用に手書き） |
| Flavor Text | フレーバーテキスト（世界観・雰囲気用。効果には影響せず、詳細モーダル最下部に斜体で表示。空欄なら非表示） |
| 読み上げ話者 | フレーバー読み上げ音声の VOICEVOX 話者。「（共通設定を使う）」＝生成ツールの既定話者。カードごとに声を変えたいときだけ指定する（音声生成については [../README.md](../README.md) 参照） |
...
- **SummonFromGrave**: 値1=出す体数（0=1体・値2不使用）。発動側**自身の墓地**からキャラカード（`CharacterCardData`）を抽出する（`GraveyardView.GetCardDataSnapshot`）。`SummonFromDeckByKeyword` のデッキ版に対する墓地版で、特徴フィルタはなく墓地のキャラ全部が候補。**フィールドの空きスロット数（`MaxCharacters` − 現在キャラ数）で出す体数の上限を打つ**（墓地から取り除いたのに場に出せず消える事故を防ぐ）。候補が体数以下なら全部・体数より多いときは複数選択ピッカー（`WaitForPlayerCardsPickAsync`。`AddToHandFromDeckByKeyword` と共通の選択 UI）から選ぶ。CPU は高コスト順、オンライン相手は墓地内インデックス**配列**を受信（`NGS_DamageTarget` チャネルを流用）。抜いたカードは `RemoveCardAt` がインデックスを詰めるため降順で取り除いてから `SummonSingleCharAsync` で配置（`OnEnter`・キャラ8体勝利判定も発動）。墓地の並び順は両クライアントで同期済み（`Recover` の `TakeFromTop` と同じ前提）のため決定的に対称解決する。墓地にキャラがいない／フィールド満杯なら空振り。
- **AddToHandFromGrave**: 値1=手札に加える枚数（0=1枚・値2不使用）。発動側**自身の墓地**の全カード（`CharacterCardData` / `EventCardData` 問わず）を候補にする（`GraveyardView.GetCardDataSnapshot`）。`SummonFromGrave` の手札版で、**場に出さないため `OnEnter` は発動せず**、特徴フィルタもなく墓地の全カードが候補（イベントカードも対象）。候補が値1以下なら全部・値1より多いときは複数選択ピッカー（`WaitForPlayerCardsPickAsync`。`SummonFromGrave` / `AddToHandFromDeckByKeyword` と共通の選択 UI）から選ぶ。CPU は高コスト順、オンライン相手は墓地内インデックス**配列**を受信（`NGS_DamageTarget` チャネルを流用）。抜いたカードは `RemoveCardAt` がインデックスを詰めるため降順で取り除いてから墓地位置から手札へ飛来する（`HandView.AddCardAnimatedAsync`・相手側は `PlayCpuDrawAsync`）。**手札が上限（8枚）に達したら超過分は墓地へバーン**（`BurnDrawnCardAsync`。Draw と同じ挙動で墓地へ戻る）。墓地にカードがなければ空振り。墓地の並び順は両クライアントで同期済み（`SummonFromGrave` / `Recover` と同じ前提）のため決定的に対称解決する（追加同期はインデックス配列のみ）。
