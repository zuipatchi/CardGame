...
| RecoverByColorChars | 自分の場の**値1指定属性のキャラ数 N** を数え、自分の墓地の上から N 枚をデッキへ戻してシャッフルする（Recover の動的枚数版・ミルなし）。値1=属性番号（白1/青2/緑3/黄4/赤5/黒6/紫7。**0=全キャラ**）。指定属性0体／範囲外なら空振り。墓地が N 枚未満なら全枚数 | 値1=属性番号 / 値2=未使用 |
...
| DamageOwnDeck | 発動側**自身のデッキ**の上から**値1枚**を墓地へ送る（セルフミル）。各ミルは DamageEnemyDeck と同じ（自分のダメージトリガー起動・自分のオーバーリミット敗北を誘発）。墓地肥やしや自デッキのダメージトリガー起動コンボに使う両刃 | 値1=ミル枚数 |
...
| AtkBoost | **自フィールドのキャラを値1体**（**値1=0 は味方全員**）選び、攻撃力を**値2分永続的に上げる**。自キャラ0体なら空振り。CPU は攻撃力上位を選ぶ。対象は ATK ラベルがオレンジにパルスし、「攻撃力 +N」フローティングラベル（`attack-buff-label`）を表示する | 値1=対象数（0=場の味方全員） / 値2=攻撃力の上昇量 |
...
| GrantKeyword | AtkBoost と同じ対象選択で、選んだ味方に**値2のキーワード能力を永続付与**（場を離れるまで）。**値1=0 は味方全員**。既に持つ能力への再付与はアイコンのポップ演出のみ。**値2が範囲外（1〜8以外。7=射手は廃止済みの欠番）なら空振り** | 値1=対象数（0=場の味方全員） / 値2=付与する能力（1=守護・2=速攻・3=飛行・4=防人・5=強襲・6=デッキ攻撃×・8=必殺） |
| HandCollectionWin | **タロー勝利（特殊勝利）**。**値1（テキスト）にカンマ区切りで書いた完全ID**（例 `C1001,E2003,C1005`）が発動時に**発動側の手札に全てそろっていれば即勝利**（勝因「タロー勝利／タロー敗北」）。**重複指定は枚数分の所持が必要**。そろわなければ空振り。発動カード自身は判定時に手札を離れているため集める対象と別 | 値1=勝利条件カードID（カンマ区切り完全ID・テキスト） / 値2=未使用 |
...
|---|---|
| None | （空文字） |
| Draw | カードをn枚引く（値2=1 のとき「（デッキが0枚の時にはカードを引かず、敗北もしない）」を付記） |
...
| RecoverByColorChars | 自分の場の{属性}のキャラの数だけ墓地の上のカードをデッキに戻してシャッフルする（{属性}は値1=白1/青2/緑3/黄4/赤5/黒6/紫7。値1=0は属性なしで「自分の場のキャラ」。範囲外は「指定属性」） |
...
| GrantKeyword | 自分のキャラn体に{能力}を付与する（n=0は場の味方全体「自分のキャラ全体に{能力}を付与する」。{能力}は m=1守護/2速攻/3飛行/4防人/5強襲/6デッキ攻撃×/8必殺。範囲外は空文字） |
...
| Flying | **飛行**。ON にすると、このキャラは守護を無視して攻撃対象（相手キャラ・相手デッキ）を選べる（攻撃側の能力）。防御側としての被攻撃制限はなく、飛行キャラも誰からでも攻撃対象になる。`EffectType` とは独立したフラグで、攻撃のみに作用する。カードと詳細モーダルに FlyIcon を表示（詳細は [rules.md](rules.md)「攻撃の詳細」） |
| Sakimori | **防人**。対空ガード。ON にすると、① 飛行はこのキャラがいる間は防人を優先して攻撃せねばならず（守護は無視するが防人は無視できない）、② 非飛行は防人だけでは縛られない（防人は地上の壁にならない。守護がいる場合は守護の対象強制に従い、狙えるのは守護のみで防人は対象に取れない）。`EffectType` とは独立したフラグで、攻撃のみに作用する。カードと詳細モーダルに ProtectorIcon を表示（詳細は [rules.md](rules.md)「攻撃の詳細」） |
...
- **DamageEnemyDeck / DamageOwnDeck / DamageBothDecks（デッキへのダメージ）**: デッキの上から指定枚数を墓地へ送る（ミル）。デッキ攻撃（`ExecuteDeckAttackAsync`）と同じ `MillDeckAsync` を経由するため、**ダメージトリガーカードの起動・オーバーリミット判定・オンライン同期**がすべて共通。`DamageEnemyDeck` は相手デッキのみ（`MillDeckAsync(deckOwnerIsLocal: !inv.IsLocal)`）、`DamageOwnDeck` は発動側自身のデッキのみ（`MillDeckAsync(deckOwnerIsLocal: inv.IsLocal)`）、`DamageBothDecks` は自分 → 相手の順に両デッキを削る。**オーバーリミット**により、空デッキ（0枚）からさらにミルしようとした瞬間にその持ち主が敗北する（値1が残り枚数を超えるとき。`DamageOwnDeck` / `DamageBothDecks` で自分のデッキが先に尽きると自滅する両刃。空デッキの相手に当てれば敗北を誘発できる）。`DamageOwnDeck` は自分の墓地肥やしや自デッキに仕込んだダメージトリガーの能動起動に使う。デッキ順は同期済みのため決定的に対称動作する（追加同期不要）。
...
- **RecoverByColorChars（指定属性キャラ数ぶんの墓地回収）**: 値1=属性番号（白1/青2/緑3/黄4/赤5/黒6/紫7。`CardAttributeNames.FromNumber`。**0=属性を問わず自フィールドの全キャラ**＝`FieldView.Characters.Count`。範囲外なら空振り）。`DamageDeckRecoverByColorChars` から**相手デッキへのミルを除いた回収のみ版**で、自フィールドのその属性のキャラ数 N を数え、`ApplyRecoverEffectAsync` で自分の墓地の上から N 枚をデッキへ戻してシャッフルする（`ApplyRecoverByCharCountAsync`）。墓地回収は `Recover` と同じ（墓地が N 枚未満なら全枚数・オンラインはデッキ順を同期）で、**演出は `Recover` と同じ「回復+N」フローティングラベル（`recover-label`）＋回復パーティクル**を発動カード上に表示する。キャラ0体なら空振り。盤面・墓地・デッキ順は同期済みのため決定的に対称解決する（追加同期不要）。
...
- **GrantKeyword（キーワード能力付与）**: 発動側が自フィールドから値1体（**値1=0 は場の味方全員**・対象数が味方数以上なら全員）を選び、**値2が示すキーワード能力**（1=守護・2=速攻・3=飛行・4=防人・5=強襲・6=デッキ攻撃×・8=必殺。7=射手は廃止済みの欠番。`GrantableKeyword`）を永続付与する。これらは従来 `CharacterCardData`（SO）固有フラグから直接読んでいたが、実行時付与のため `CardView` に付与フラグと `HasGuardian` / `HasHaste` / `HasFlying` / `HasSakimori` / `HasAssault` / `HasNoDeckAttack` / `HasDeadly`（SO 固有 OR 付与）プロパティを新設し、攻撃ルール判定（`MainPresenter.Phases.Main` の `IsGuardian` 等）・戦闘解決（必殺）・盤面アイコン・`CopyFieldChar`（`CopyRuntimeStateFrom`）をこのプロパティ経由に統一した。対象選択は AtkBoost / HpBoost と同じ `ResolveAllyCharTargetsAsync`（プレイヤー選択／CPU は攻撃力上位／オンラインはインデックス同期）を共用。付与は値2（カードデータ）と同期済み盤面から決定的に解決されるため、オンライン・CPU でも対称に発動する（追加同期不要）。値2が範囲外（1〜8以外・7=射手は廃止）／自キャラ0体なら空振り。**カード詳細モーダルは攻撃力/HP バフ同様カード固有能力のみ表示し、実行時付与は反映しない**（盤面のアイコンには反映される）。
