...
|---|---|---|
| None | 効果なし | － |
| Draw | 発動プレイヤーが指定枚数デッキ上から手札に加える。**オーバーリミット**：1枚引く直前にデッキが空（0枚）なら、その瞬間に発動プレイヤーがデッキ切れで敗北する（2枚引く効果でデッキ1枚なら、1枚引いてデッキ0→2枚目を引こうとして敗北。[rules.md](rules.md)「勝敗条件」）。手札が上限（8枚）の場合は引いたカードを墓地へ送る（バーン。[rules.md](rules.md)「手札の上限」）。**値2は将来の拡張用に予約**（現状は不使用） | 値1=ドロー枚数 / 値2=予約・未使用 |
| CoinDraw | コインを振り、**表が出るたびにカードを1枚引き、裏が出たら終了**する（乱数で判定。引く枚数は0枚もありうる）。1トスごとにカード付近でコイン演出を再生する（回転中は Coin SE を鳴らし回転終了で停止）。**引く枚数はデッキ残り枚数で上限を打つ**ため、空デッキから引いて強制敗北することはない（最後の1枚を引いてデッキが0になったときのリミットブレイク告知は通常 Draw と同じ）。手札上限バーンも通常 Draw と同じ。**オンライン**：発動側が振った「表の回数」だけを相手へ送信し（`NGS_CoinDraw`）、ミラー側は同期済みデッキの上から同じ枚数を引く（デッキ順が同期しているため同じカードになる） | 値1・値2とも不使用（0） |
