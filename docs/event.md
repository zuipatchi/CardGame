...
| GrantKeyword | 自分のキャラn体に{能力}を付与する（n=0は場の味方全体「自分のキャラ全体に{能力}を付与する」。{能力}は m=1守護/2速攻/3飛行/4防人/5強襲/6デッキ攻撃×/7射手。範囲外は空文字） |
...
- **GrantKeyword（キーワード能力付与）**: 発動側が自フィールドから値1体（**値1=0 は場の味方全員**・対象数が味方数以上なら全員）を選び、**値2が示すキーワード能力**（1=守護・2=速攻・3=飛行・4=防人・5=強襲・6=デッキ攻撃×・7=射手。`GrantableKeyword`）を永続付与する。これらは従来 `CharacterCardData`（SO）固有フラグから直接読んでいたが、実行時付与のため `CardView` に付与フラグと `HasGuardian` / `HasHaste` / `HasFlying` / `HasSakimori` / `HasAssault` / `HasNoDeckAttack` / `HasArcher`（SO 固有 OR 付与）プロパティを新設し、攻撃ルール判定（`MainPresenter.Phases.Main` の `IsGuardian` 等）・盤面アイコン・`CopyFieldChar`（`CopyRuntimeStateFrom`）をこのプロパティ経由に統一した。対象選択は AtkBoost / HpBoost と同じ `ResolveAllyCharTargetsAsync`（プレイヤー選択／CPU は攻撃力上位／オンラインはインデックス同期）を共用。付与は値2（カードデータ）と同期済み盤面から決定的に解決されるため、オンライン・CPU でも対称に発動する（追加同期不要）。値2が範囲外（1〜7以外）／自キャラ0体なら空振り。**カード詳細モーダルは攻撃力/HP バフ同様カード固有能力のみ表示し、実行時付与は反映しない**（盤面のアイコンには反映される）。
