...
|---|---|---|
| None | 効果なし | － |
| Draw | 発動プレイヤーが指定枚数デッキ上から手札に加える。**オーバーリミット**：1枚引く直前にデッキが空（0枚）なら、その瞬間に発動プレイヤーがデッキ切れで敗北する（2枚引く効果でデッキ1枚なら、1枚引いてデッキ0→2枚目を引こうとして敗北。[rules.md](rules.md)「勝敗条件」）。手札が上限（8枚）の場合は引いたカードを墓地へ送る（バーン。[rules.md](rules.md)「手札の上限」） | ドロー枚数 |
