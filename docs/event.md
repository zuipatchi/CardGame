...
| DebuffAttack | 発動側から見た敵キャラを**値1体**（**値1=0 は敵全員**）選び、それぞれの攻撃力を**値2分、永続的に下げる**（発動時に一度だけ減算）。**攻撃力は0未満にならない**（`CurrentAttack` を 0 でクランプ）。対象はプレイヤーがクリックで選択（DamageEnemy と同じ `selectable-char` / `selected-char` ハイライト・残り体数トースト）。対象数が敵の数以上なら全員が対象（選択不要）・敵キャラ0体／値2≤0なら空振り。CPU は攻撃力上位を狙う。オンラインは対象をフィールドのインデックス配列で同期（`NGS_DamageTarget`）。対象は ATK ラベルが青にパルスし、各対象の上に「攻撃ダウンN」フローティングラベル（`debuff-attack-label`）が出る。AtkBoost の敵版・マイナス版 | 値1=対象数（0=敵全員） / 値2=攻撃力の減少量 |
...
| DamageDeckRecoverByColorChars | **自分の場の値1で指定した属性のキャラの数 N を数え、相手デッキに N ダメージ（上から N 枚ミル）を与え、同じ N 枚を自分の墓地の上からデッキへ戻してシャッフルする**。値1=属性番号（白1/青2/緑3/黄4/赤5/黒6/紫7。**0=属性を問わず自分の場の全キャラ**。範囲外なら空振り）。ミルは DamageEnemyDeck と同じ（ダメージトリガー・オーバーリミット判定込み）。墓地への戻しは Recover と同じ（墓地が N 枚未満なら全枚数・オンラインはデッキ順を同期）。N は同期済み盤面から決定的に算出（追加同期不要）。指定属性キャラが0体なら空振り。**ミルで相手がデッキ切れ敗北してゲームが終わったら墓地の戻しは行わない** | 値1=属性番号 / 値2=未使用 |
...
| DrawSkipNext | 発動プレイヤーが**値1**枚ドローし（Draw と同じ。デッキ0枚で中断。**値1=0 なら引かない**）、ドローフェーズを1回スキップする（ドロー0枚にして消費。スキップ時は「ドロースキップ」トースト表示）。スキップ対象は**値2**で切り替え＝**0＝発動プレイヤー自身**の次のドローフェーズ、**1＝相手**の次のドローフェーズ。スキップ予約はターンをまたいで次に来る対象プレイヤーのドローフェーズで消費される（ExtraTurn 中でも「次に来るドロー」を飛ばす）。オンラインは各クライアントが自分側のフラグを立てて対称に解決（追加同期なし） | 値1=ドロー枚数（0で引かない） / 値2=スキップ対象（0=自分 / 1=相手） |
...
| SummonFromGrave | 発動側**自身の墓地**からキャラカードを**値1**体（未設定=0 は1体）選んで自フィールドに出す（墓地から消費）。候補（墓地のキャラ）が値1以下なら全部・値1より多いときはピッカー（候補カードを並べたオーバーレイ）から値1体タップで選択（再タップで解除・値1体で確定）。CPU は高コスト順、オンライン相手は墓地内インデックス配列で同期（`NGS_DamageTarget` チャネルを流用）。配置時に `OnEnter` も発動する。**フィールドの空きスロット数で上限を打つ**（墓地から抜いたのに場に出せず消えないように）。墓地にキャラがいない／フィールド満杯なら空振り。墓地の並び順は両クライアントで同期済み（`Recover` と同じ前提）のためインデックス指定で決定的に解決 | 値1=出す体数（0=1体） / 値2=未使用 |
...
| GrantKeyword | AtkBoost と同じ対象選択で、選んだ味方に**値2で指定したキーワード能力（守護/速攻/飛行/防人/強襲/デッキ攻撃×/射手）を永続付与する**（場を離れるまで持続）。**値1=0 は場の味方キャラ全員が対象**（選択不要）。付与した能力は実行時に `CardView` が保持し（`Has*` プロパティ）、攻撃ルール判定・盤面アイコン・`CopyFieldChar` のコピーに反映される。既に持っている能力への再付与はアイコンのポップ演出のみ。**値2が範囲外（1〜7以外）なら空振り**。付与は値2（カードデータ）＋対象インデックス同期で決定的に解決（追加同期不要） | 値1=対象数（0=場の味方全員） / 値2=付与する能力（1=守護・2=速攻・3=飛行・4=防人・5=強襲・6=デッキ攻撃×・7=射手） |
...
- **GrantKeyword（キーワード能力付与）**: 発動側が自フィールドから値1体（**値1=0 は場の味方全員**・対象数が味方数以上なら全員）を選び、**値2が示すキーワード能力**（1=守護・2=速攻・3=飛行・4=防人・5=強襲・6=デッキ攻撃×・7=射手。`GrantableKeyword`）を永続付与する。守護/速攻/飛行/防人/強襲/デッキ攻撃×/射手は従来 `CharacterCardData`（SO）固有フラグから直接読んでいたが、実行時付与のため `CardView` に付与フラグと `HasGuardian` / `HasHaste` / `HasFlying` / `HasSakimori` / `HasAssault` / `HasNoDeckAttack` / `HasArcher`（SO 固有 OR 付与）プロパティを新設し、攻撃ルール判定（`MainPresenter.Phases.Main` の `IsGuardian` 等）・盤面アイコン・`CopyFieldChar`（`CopyRuntimeStateFrom`）をこのプロパティ経由に統一した。対象選択は AtkBoost / HpBoost と同じ `ResolveAllyCharTargetsAsync`（プレイヤー選択／CPU は攻撃力上位／オンラインはインデックス同期）を共用。付与は値2（カードデータ）と同期済み盤面から決定的に解決されるため、オンライン・CPU でも対称に発動する（追加同期不要）。値2が範囲外（1〜7以外）／自キャラ0体なら空振り。**カード詳細モーダルは攻撃力/HP バフ同様カード固有能力のみ表示し、実行時付与は反映しない**（盤面のアイコンには反映される）。
