...
|---|---|---|
| None | 効果なし | － |
| Draw | 発動プレイヤーが指定枚数デッキ上から手札に加える。デッキが 0 枚になった時点で残りのドローを中断（ゲームオーバーにはならない）。手札が上限（8枚）の場合は引いたカードを墓地へ送る（バーン。[rules.md](rules.md)「手札の上限」） | ドロー枚数 |
...
| Switch | 発動プレイヤー自身のフィールドのキャラを1体選んで（複数いる場合はクリックで選択）手札に戻し、手札からキャラカードを1枚コストを払って配置する。フィールドにキャラがいない場合は効果なし。手札が上限（8枚）のとき戻すキャラは墓地へ送る（バーン。[rules.md](rules.md)「手札の上限」） | 使用しない（0 固定） |
...
| GrantKeyword | AtkBoost と同じ対象選択で、選んだ味方に**値2で指定したキーワード能力（守護/速攻/飛行/防人）を永続付与する**（場を離れるまで持続）。**値1=0 は場の味方キャラ全員が対象**（選択不要）。付与した能力は実行時に `CardView` が保持し（`Has*` プロパティ）、攻撃ルール判定・盤面アイコン・`CopyFieldChar` のコピーに反映される。既に持っている能力への再付与はアイコンのポップ演出のみ。**値2が範囲外（1〜4以外）なら空振り**。付与は値2（カードデータ）＋対象インデックス同期で決定的に解決（追加同期不要） | 値1=対象数（0=場の味方全員） / 値2=付与する能力（1=守護・2=速攻・3=飛行・4=防人） |
...
| GrantKeyword | 自分のキャラn体に{能力}を付与する（n=0は場の味方全体「自分のキャラ全体に{能力}を付与する」。{能力}は m=1守護/2速攻/3飛行/4防人。範囲外は空文字） |
...
- **GrantKeyword（キーワード能力付与）**: 発動側が自フィールドから値1体（**値1=0 は場の味方全員**・対象数が味方数以上なら全員）を選び、**値2が示すキーワード能力**（1=守護・2=速攻・3=飛行・4=防人。`GrantableKeyword`）を永続付与する。守護/速攻/飛行/防人は従来 `CharacterCardData`（SO）固有フラグから直接読んでいたが、実行時付与のため `CardView` に付与フラグと `HasGuardian` / `HasHaste` / `HasFlying` / `HasSakimori`（SO 固有 OR 付与）プロパティを新設し、攻撃ルール判定（`MainPresenter.Phases.Main` の `IsGuardian` 等）・盤面アイコン・`CopyFieldChar`（`CopyRuntimeStateFrom`）をこのプロパティ経由に統一した。対象選択は AtkBoost / HpBoost と同じ `ResolveAllyCharTargetsAsync`（プレイヤー選択／CPU は攻撃力上位／オンラインはインデックス同期）を共用。付与は値2（カードデータ）と同期済み盤面から決定的に解決されるため、オンライン・CPU でも対称に発動する（追加同期不要）。値2が範囲外（1〜4以外）／自キャラ0体なら空振り。**カード詳細モーダルは攻撃力/HP バフ同様カード固有能力のみ表示し、実行時付与は反映しない**（盤面のアイコンには反映される）。
