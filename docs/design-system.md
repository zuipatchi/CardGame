...
閲覧系モーダル（カード詳細・墓地一覧など）はパネル右上に `×` 閉じるボタンを `position: absolute` で置く（外側クリックだけに頼らない確実な閉じ導線）。オンライン相手ターン中のフレームヒッチで `ClickEvent` が合成されないことがあるため、外側クリックでの閉じは `PointerDownEvent` を使う（[CardDetailModal.cs](../Assets/Scripts/Main/Card/CardDetailModal.cs) が参考実装）。

効果解決中のカード選択ピッカー（デッキ/墓地から召喚・手札に加える・ハンデス）の操作は選択方式で分かれる。**複数選択ピッカー**（共通の `WaitForPlayerCardsPickAsync`＝手札に加える・墓地から召喚・ハンデス）は、カード**左上のチェックボックス**（`deck-pick-check`）のタップで選択/解除し、必要枚数に達するとピッカー下部の「決定」ボタンで確定する（カード詳細を踏まずに選択できる。共通ヘルパー `AddPickerCardSelection`）。**カード本体**のタップは詳細モーダルを**閲覧用**に開く（選択はしない）。選択状態はカードの枠ハイライト＋チェックボックスの塗りつぶしで示す。**単体選択ピッカー**（デッキから召喚）は従来どおりカード本体のタップで詳細モーダルを開き、下部のアクションボタンで即決定する。
