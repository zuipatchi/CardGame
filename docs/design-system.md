...
効果解決中のカード選択ピッカー（デッキ/墓地から召喚・手札に加える・ハンデス）は、カードのクリックで**詳細モーダルを開き**、詳細パネル下部のアクションボタンで決定する（単体選択は即決定、複数選択は選択/解除をトグルしてピッカー下部の「決定」ボタンで確定）。一覧上のカードはホバー・選択で拡大せず、選択は枠ハイライトのみで示す（モーダルからのはみ出し防止）。
