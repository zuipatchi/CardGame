...
namespace Main.Card
{
    ...
    public sealed class EventCardSO : ScriptableObject
    {
        ...
        // 所属カードの属性・弾を SO の値に揃えたうえで、属性×弾ごとに E#### を自動採番する
        ...
        private void OnValidate()
        {
            ...
            if (CardIdAutoAssigner.AssignIds(_cards, "E", _set))
            {
                changed = true;
            }
            if (changed)
        ...
        public void EditorSetCards(CardAttribute attribute, List<EventCardData> cards, int set = 1)
        {
            ...
            ApplySetToCards();
            CardIdAutoAssigner.AssignIds(_cards, "E", _set);
        }
#endif
    }
}
