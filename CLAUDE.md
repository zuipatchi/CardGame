...
| サウンド再生（BGM/SE/ボイス読み上げ。PlayVoice は前の読み上げを止めず重ねて再生。PlaySE は SE ごとの正規化倍率を掛けて再生。PlayLoopSE/StopLoopSE は途中で止められる持続 SE 用＝コイントス回転音など） | [Assets/Scripts/Common/SoundManagement/SoundPlayer.cs](Assets/Scripts/Common/SoundManagement/SoundPlayer.cs) |
| BGM/SE クリップのロード＋SE ラウドネス正規化（ロード時に各 SE の RMS を解析し同じ聞こえ方になる音量倍率を算出。手動微調整は `ManualSeAdjust`） | [Assets/Scripts/Common/Store/SoundStore.cs](Assets/Scripts/Common/Store/SoundStore.cs) |
