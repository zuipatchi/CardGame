...
| サウンド再生（BGM/SE/ボイス読み上げ・持続 SE） | [Assets/Scripts/Common/SoundManagement/SoundPlayer.cs](Assets/Scripts/Common/SoundManagement/SoundPlayer.cs) |
| BGM/SE クリップのロード＋SE ラウドネス正規化（手動微調整は `ManualSeAdjust` / `ManualBgmAdjust`） | [Assets/Scripts/Common/Store/SoundStore.cs](Assets/Scripts/Common/Store/SoundStore.cs) |
| ボリューム状態モデル（マスター/BGM/SE/ボイス） | [Assets/Scripts/Common/Option/OptionModel.cs](Assets/Scripts/Common/Option/OptionModel.cs) |
...
| WebGL canvas のタッチ設定（`touch-action: none` を実行時設定・jslib） | [Assets/Scripts/Common/Platform/WebGLTouchAction.cs](Assets/Scripts/Common/Platform/WebGLTouchAction.cs) / [Assets/Plugins/WebGL/TouchAction.jslib](Assets/Plugins/WebGL/TouchAction.jslib) |
...
| チュートリアル起動情報（Common・消費型フラグ＋TutorialId） | [Assets/Scripts/Common/Tutorial/TutorialModel.cs](Assets/Scripts/Common/Tutorial/TutorialModel.cs) |
...
| デッキ保存（9スロット制・PlayerPrefs。旧単一デッキはスロット1へ自動移行。初回起動時のシードも担当） | [Assets/Scripts/Common/Deck/DeckRepository.cs](Assets/Scripts/Common/Deck/DeckRepository.cs) |
| 初期デッキ定義 SO（初回起動時にシードする中身。カードIDのみ。Addressables `Deck/StarterDeck`） | [Assets/Scripts/Common/Deck/StarterDeckSO.cs](Assets/Scripts/Common/Deck/StarterDeckSO.cs) |
| 初期デッキ自動生成（Common 常駐・初回起動時のみスロット1へシード。既存デッキ保有者は上書きせずフラグのみ） | [Assets/Scripts/Common/Deck/StarterDeckSeeder.cs](Assets/Scripts/Common/Deck/StarterDeckSeeder.cs) |
...
| CPU デッキ SO（ロスター未設定時のフォールバック） | [Assets/Scripts/Main/Card/CpuDeckSO.cs](Assets/Scripts/Main/Card/CpuDeckSO.cs) |
| CPU 相手ロスター SO（相手ごとの名前/ポートレート/難易度/デッキ） | [Assets/Scripts/Common/Cpu/CpuRosterSO.cs](Assets/Scripts/Common/Cpu/CpuRosterSO.cs) |
| CPU 相手ロスターのロード（Common 常駐ストア） | [Assets/Scripts/Common/Cpu/CpuRosterStore.cs](Assets/Scripts/Common/Cpu/CpuRosterStore.cs) |
| 選んだ CPU 相手の受け渡し（Common・index 保持） | [Assets/Scripts/Common/Cpu/CpuBattleModel.cs](Assets/Scripts/Common/Cpu/CpuBattleModel.cs) |
...
| デッキ構築 Presenter（スロット一覧↔編集の2画面・シンボル設定・使用デッキ選択） | [Assets/Scripts/DeckBuilder/DeckBuilderPresenter.cs](Assets/Scripts/DeckBuilder/DeckBuilderPresenter.cs) |
...
| CPU デッキ構築 Presenter（相手スロット式・Editor 専用） | [Assets/Scripts/CpuDeckEditor/CpuDeckBuilderPresenter.cs](Assets/Scripts/CpuDeckEditor/CpuDeckBuilderPresenter.cs) |
...
| Home Presenter（CPU相手選択・使用デッキ選択・ユーザーネーム編集の各モーダル） | [Assets/Scripts/Home/HomePresenter.cs](Assets/Scripts/Home/HomePresenter.cs) |
...
| デッキ構築 BGM 管理 | [Assets/Scripts/DeckBuilder/Sound/DeckBuilderAudioManager.cs](Assets/Scripts/DeckBuilder/Sound/DeckBuilderAudioManager.cs) |
| マッチング BGM 管理 | [Assets/Scripts/Matching/Sound/MatchingAudioManager.cs](Assets/Scripts/Matching/Sound/MatchingAudioManager.cs) |
...
| マッチング Presenter（使用デッキ選択：ルーム選択カード上の使用デッキボタン＋9スロット選択モーダル・シンボル画像表示） | [Assets/Scripts/Matching/MatchingPresenter.DeckSelect.cs](Assets/Scripts/Matching/MatchingPresenter.DeckSelect.cs) |
...
| カードデータ基底（読み上げ話者 `_voiceSpeaker` 含む） | [Assets/Scripts/Main/Card/CardData.cs](Assets/Scripts/Main/Card/CardData.cs) |
...
| 付与キーワード能力 enum（GrantKeyword の値2対応・守護/速攻/飛行/防人/強襲/デッキ攻撃×/必殺。値7=射手は廃止済みの欠番） | [Assets/Scripts/Main/Card/GrantableKeyword.cs](Assets/Scripts/Main/Card/GrantableKeyword.cs) |
| カードID自動採番（属性×弾別・Editor 専用） | [Assets/Scripts/Main/Card/CardIdAutoAssigner.cs](Assets/Scripts/Main/Card/CardIdAutoAssigner.cs) |
| カード編集ウィンドウ（検索・編集・追加・削除・ID再採番・効果テキスト一括生成。Editor 専用。詳細は [docs/event.md](docs/event.md)） | [Assets/Scripts/Editor/CardEditorWindow.cs](Assets/Scripts/Editor/CardEditorWindow.cs) |
...
| 属性別SO分割ツール（旧単一SO→属性別 SO への一度きりの移行ツール。Editor 専用） | [Assets/Scripts/Editor/CardSoAttributeSplitter.cs](Assets/Scripts/Editor/CardSoAttributeSplitter.cs) |
...
| カード詳細モーダル（キーワード説明展開・任意のアクションボタン・閉時 `OnHidden` 通知） | [Assets/Scripts/Main/Card/CardDetailModal.cs](Assets/Scripts/Main/Card/CardDetailModal.cs) |
...
| 共通の勝利条件ルール（デッキ切れ＝オーバーリミット/勝利点20） | [Assets/Scripts/Main/Game/WinRule.cs](Assets/Scripts/Main/Game/WinRule.cs) |
| 初期手札枚数ルール（先攻3枚/後攻4枚） | [Assets/Scripts/Main/Game/MulliganRule.cs](Assets/Scripts/Main/Game/MulliganRule.cs) |
...
| Main Presenter（リザルト画面の使用デッキ変更：使用デッキボタン＋9スロット選択モーダル・シンボル画像・再戦可否判定） | [Assets/Scripts/Main/MainPresenter.DeckSelect.cs](Assets/Scripts/Main/MainPresenter.DeckSelect.cs) |
| Main Presenter（チュートリアル：誘導つきスクリプト対戦。詳細は [docs/architecture.md](docs/architecture.md)「チュートリアル」） | [Assets/Scripts/Main/MainPresenter.Tutorial.cs](Assets/Scripts/Main/MainPresenter.Tutorial.cs) |
| Main Presenter（ゲームループ・InitializeFirstTurn・OnGameEnd・守護/防人の自動タップ） | [Assets/Scripts/Main/MainPresenter.Phases.cs](Assets/Scripts/Main/MainPresenter.Phases.cs) |
| Main Presenter（ドローフェーズ） | [Assets/Scripts/Main/MainPresenter.Phases.Draw.cs](Assets/Scripts/Main/MainPresenter.Phases.Draw.cs) |
| Main Presenter（メインフェーズ・CPU選択/難易度・タップ/アンタップ・攻撃可否判定・行動予約） | [Assets/Scripts/Main/MainPresenter.Phases.Main.cs](Assets/Scripts/Main/MainPresenter.Phases.Main.cs) |
...
| Main Presenter（効果適用：全体/単体ダメージ・消滅・攻撃力ダウン（DebuffAttack・0クランプ）＋敵対象選択） | [Assets/Scripts/Main/MainPresenter.Phases.Resolution.Damage.cs](Assets/Scripts/Main/MainPresenter.Phases.Resolution.Damage.cs) |
...
| Main Presenter（自分の墓地からキャラをn体選んで場に出す） | [Assets/Scripts/Main/MainPresenter.Phases.Resolution.SummonFromGrave.cs](Assets/Scripts/Main/MainPresenter.Phases.Resolution.SummonFromGrave.cs) |
| Main Presenter（デッキから特徴指定でn枚手札に加える・複数選択ピッカー） | [Assets/Scripts/Main/MainPresenter.Phases.Resolution.AddToHandFromDeck.cs](Assets/Scripts/Main/MainPresenter.Phases.Resolution.AddToHandFromDeck.cs) |
| Main Presenter（自分の墓地からカードをn枚選んで手札に加える・複数選択ピッカー） | [Assets/Scripts/Main/MainPresenter.Phases.Resolution.AddToHandFromGrave.cs](Assets/Scripts/Main/MainPresenter.Phases.Resolution.AddToHandFromGrave.cs) |
...
| Main Presenter（ハンデス：相手が手札を捨てる・持ち主が選ぶ） | [Assets/Scripts/Main/MainPresenter.Phases.Resolution.Discard.cs](Assets/Scripts/Main/MainPresenter.Phases.Resolution.Discard.cs) |
...
| Main Presenter（告知・トースト） | [Assets/Scripts/Main/MainPresenter.Animations.cs](Assets/Scripts/Main/MainPresenter.Animations.cs) |
| Main Presenter（コイントス演出・コインドローのトス演出・サイコロドローのダイス演出） | [Assets/Scripts/Main/MainPresenter.Animations.Coin.cs](Assets/Scripts/Main/MainPresenter.Animations.Coin.cs) |
...
| Main Presenter（カード飛行・ドロー・墓地公開ドロー・召喚登場演出・ダメージトリガー発動カットイン） | [Assets/Scripts/Main/MainPresenter.Animations.CardFly.cs](Assets/Scripts/Main/MainPresenter.Animations.CardFly.cs) |
...
| Main USS（盤面レイアウト／演出別に4分割：Overlays・FloatingLabels・Highlights・Tutorial） | [Assets/Scripts/Main/View/](Assets/Scripts/Main/View/) |
...
| 日本語フォント（UI Toolkit デフォルト＝Noto Sans JP Bold。個別指定不要） | [Assets/Font/](Assets/Font/) |
| UI デフォルトフォント設定（Bold FontAsset + PanelTextSettings 生成・Panel Settings へ割当。メニュー `Tools/UI/Setup Bold Japanese Font`・Editor 専用） | [Assets/Scripts/Editor/UiFontSetup.cs](Assets/Scripts/Editor/UiFontSetup.cs) |
