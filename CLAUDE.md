...
| フレーバー音声一括生成ツール（VOICEVOX 連携・Addressables 自動登録・カードと犬セリフの両方を生成・Editor 専用） | [Assets/Scripts/Editor/FlavorVoiceGeneratorWindow.cs](Assets/Scripts/Editor/FlavorVoiceGeneratorWindow.cs) |
...
| Home Presenter（CPU相手選択カルーセル・各モーダルの右上×ボタン含む） | [Assets/Scripts/Home/HomePresenter.cs](Assets/Scripts/Home/HomePresenter.cs) |
...
| Home 犬セリフ Presenter（吹き出し表示＋VOICEVOX 音声読み上げ） | [Assets/Scripts/Home/DogSpeechPresenter.cs](Assets/Scripts/Home/DogSpeechPresenter.cs) |
| Home 犬セリフ SO（セリフ＋読み上げ話者 ID） | [Assets/Scripts/Home/DogSpeechLinesSO.cs](Assets/Scripts/Home/DogSpeechLinesSO.cs) |
| Home 犬セリフ音声キー算出（本文→安定キー＋読み上げ用テキスト変換。ランタイムと生成ツールで共有） | [Assets/Scripts/Home/DogVoice.cs](Assets/Scripts/Home/DogVoice.cs) |
| Home 犬セリフ音声ロード（Addressables `Voice/Dog/{key}` をオンデマンド・キャッシュ） | [Assets/Scripts/Home/DogVoiceStore.cs](Assets/Scripts/Home/DogVoiceStore.cs) |
...
| カード詳細モーダル（キーワード能力アイコンをタップで効果説明を展開） | [Assets/Scripts/Main/Card/CardDetailModal.cs](Assets/Scripts/Main/Card/CardDetailModal.cs) |
