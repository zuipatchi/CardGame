...
| カード編集ウィンドウ（検索・編集・追加・削除・並び替え（ID/コスト順）・ID再採番・ゲーム使用/不使用切替・読み上げ話者の個別指定。効果テキスト一括生成はメニュー `Card → 効果テキストを一括生成` から実行。Editor 専用） | [Assets/Scripts/Editor/CardEditorWindow.cs](Assets/Scripts/Editor/CardEditorWindow.cs) |
