...
| デッキ構築 Presenter（9枠のスロット一覧↔編集の2画面・デッキ名リネーム・各枠にシンボルカード表示＝未設定はカード裏面。カード詳細の「★デッキのシンボルに設定」で代表カードを指定。スロット一覧上部の「使用デッキ」ボタン→Home風モーダルで対戦デッキ（SelectedIndex）もここから選べる） | [Assets/Scripts/DeckBuilder/DeckBuilderPresenter.cs](Assets/Scripts/DeckBuilder/DeckBuilderPresenter.cs) |
