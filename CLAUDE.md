...
- ボタンクリック模擬: `button.clicked`（Clickable）ベースなら `NavigationSubmitEvent`、`RegisterCallback<ClickEvent>` ベースなら `ClickEvent.GetPooled()` を使う
- MonoBehaviour の `Awake/OnEnable/Start` は VContainer インジェクション完了前に呼ばれる（`CommonSceneLoader.Awake()` が `async void` で NextFrame 待機後に `BuildLifetimeScopes` するため）。インジェクト済みフィールドを使う初期化は `IAsyncStartable.StartAsync()` に移すこと
...
| 戦闘計算（ATK） | [Assets/Scripts/Main/Game/BattleCalculator.cs](Assets/Scripts/Main/Game/BattleCalculator.cs) |
| Main Presenter（フィールド・BuildAsync） | [Assets/Scripts/Main/MainPresenter.cs](Assets/Scripts/Main/MainPresenter.cs) |
| Main Presenter（ゲームループ・優先権・OnGameEnd） | [Assets/Scripts/Main/MainPresenter.Phases.cs](Assets/Scripts/Main/MainPresenter.Phases.cs) |
| Main Presenter（ドローフェーズ） | [Assets/Scripts/Main/MainPresenter.Phases.Draw.cs](Assets/Scripts/Main/MainPresenter.Phases.Draw.cs) |
| Main Presenter（キャラセットフェーズ） | [Assets/Scripts/Main/MainPresenter.Phases.CharSet.cs](Assets/Scripts/Main/MainPresenter.Phases.CharSet.cs) |
| Main Presenter（PreBattle2フェーズ） | [Assets/Scripts/Main/MainPresenter.Phases.PreBattle.cs](Assets/Scripts/Main/MainPresenter.Phases.PreBattle.cs) |
| Main Presenter（解決フェーズ・イベント効果） | [Assets/Scripts/Main/MainPresenter.Phases.Resolution.cs](Assets/Scripts/Main/MainPresenter.Phases.Resolution.cs) |
| Main Presenter（戦闘フェーズ） | [Assets/Scripts/Main/MainPresenter.Phases.Battle.cs](Assets/Scripts/Main/MainPresenter.Phases.Battle.cs) |
...
| Main Presenter（コイントス・優先権コイン演出） | [Assets/Scripts/Main/MainPresenter.Animations.Coin.cs](Assets/Scripts/Main/MainPresenter.Animations.Coin.cs) |
| Main Presenter（ATKカウンター・攻撃演出） | [Assets/Scripts/Main/MainPresenter.Animations.Attack.cs](Assets/Scripts/Main/MainPresenter.Animations.Attack.cs) |
| Main Presenter（カード飛行・ドロー・デッキダメージ） | [Assets/Scripts/Main/MainPresenter.Animations.CardFly.cs](Assets/Scripts/Main/MainPresenter.Animations.CardFly.cs) |
