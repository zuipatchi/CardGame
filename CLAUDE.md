...
| カード詳細モーダル（キーワード能力アイコンをタップで効果説明を展開・任意のアクションボタンを下部に表示できる＝デッキ構築のシンボル設定で使用。右上の✕ボタンまたは外側クリックで閉じる＝閉じ判定は `PointerDownEvent`） | [Assets/Scripts/Main/Card/CardDetailModal.cs](Assets/Scripts/Main/Card/CardDetailModal.cs) |
