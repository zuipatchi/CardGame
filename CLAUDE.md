...
| EventSystem セットアップ（タッチ入力に必須・Common シーンへ追加・Editor 専用。詳細は [docs/architecture.md](docs/architecture.md)「スマホ（タッチ）でカードドラッグが効かない」） | [Assets/Scripts/Editor/TouchEventSystemSetup.cs](Assets/Scripts/Editor/TouchEventSystemSetup.cs) |
...
| デッキ保存（9スロット制・PlayerPrefs。旧単一デッキはスロット1へ自動移行） | [Assets/Scripts/Common/Deck/DeckRepository.cs](Assets/Scripts/Common/Deck/DeckRepository.cs) |
