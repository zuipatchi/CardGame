...
| デッキ構築 Presenter（9枠のスロット一覧↔編集の2画面・デッキ名リネーム・各枠にシンボルカード表示。カード詳細の「★デッキのシンボルに設定」で代表カードを指定。編集専用で対戦デッキ選択は Home 側） | [Assets/Scripts/DeckBuilder/DeckBuilderPresenter.cs](Assets/Scripts/DeckBuilder/DeckBuilderPresenter.cs) |
...
| Home Presenter（CPU相手選択カルーセル・使用デッキ選択モーダル（各行の左にシンボルカード表示）・各モーダルの右上×ボタン含む） | [Assets/Scripts/Home/HomePresenter.cs](Assets/Scripts/Home/HomePresenter.cs) |
