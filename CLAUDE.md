...
| サウンド再生（BGM/SE/ボイス読み上げ。PlayBGM は曲ごとの音量倍率を掛けて再生。PlayVoice は前の読み上げを止めず重ねて再生＝ボイスは埋もれないよう v/2 でなく等倍。PlaySE は SE ごとの正規化倍率を掛けて再生。PlayLoopSE/StopLoopSE は途中で止められる持続 SE 用＝コイントス回転音など） | [Assets/Scripts/Common/SoundManagement/SoundPlayer.cs](Assets/Scripts/Common/SoundManagement/SoundPlayer.cs) |
| BGM/SE クリップのロード＋SE ラウドネス正規化（ロード時に各 SE の RMS を解析し同じ聞こえ方になる音量倍率を算出。手動微調整は `ManualSeAdjust`）。BGM は曲ごとの音量差を手動倍率テーブル `ManualBgmAdjust`（クリップ名→倍率）で揃える | [Assets/Scripts/Common/Store/SoundStore.cs](Assets/Scripts/Common/Store/SoundStore.cs) |
| ボリューム状態モデル（マスター/BGM/SE/ボイス。ボイスは SE と独立。マスターは全チャンネルに掛かり実効倍率は値×2＝スライダー0.5で等倍） | [Assets/Scripts/Common/Option/OptionModel.cs](Assets/Scripts/Common/Option/OptionModel.cs) |
...
| デッキ保存（9スロット制・各スロットにカード列/デッキ名/シンボルカードID/対戦に使う選択中スロットを PlayerPrefs 保存。旧単一デッキはスロット1へ自動移行） | [Assets/Scripts/Common/Deck/DeckRepository.cs](Assets/Scripts/Common/Deck/DeckRepository.cs) |
...
| CPU デッキ SO（Addressables `Card/CpuDeck`・ロスター未設定/空スロット時のフォールバック） | [Assets/Scripts/Main/Card/CpuDeckSO.cs](Assets/Scripts/Main/Card/CpuDeckSO.cs) |
| CPU 相手ロスター SO（相手ごとの名前・ポートレート・難易度・デッキ＝カードID配列。Common・Addressables `Card/CpuRoster`） | [Assets/Scripts/Common/Cpu/CpuRosterSO.cs](Assets/Scripts/Common/Cpu/CpuRosterSO.cs) |
| CPU 相手ロスターのロード（Common 常駐ストア・未登録/空のときは8体プレースホルダー扱い） | [Assets/Scripts/Common/Cpu/CpuRosterStore.cs](Assets/Scripts/Common/Cpu/CpuRosterStore.cs) |
| 選んだ CPU 相手の受け渡し（Common・index 保持。Main 再ロードの再戦でも同じ相手） | [Assets/Scripts/Common/Cpu/CpuBattleModel.cs](Assets/Scripts/Common/Cpu/CpuBattleModel.cs) |
...
| デッキ構築 Presenter（9枠のスロット一覧↔編集の2画面・デッキ名リネーム・各枠にシンボルカード表示＝未設定はカード裏面。カード詳細の「★デッキのシンボルに設定」で代表カードを指定。スロット一覧上部の「使用デッキ」ボタン→Home風モーダルで対戦デッキ（SelectedIndex）もここから選べる。編集画面の「デッキ」見出し横の「使用する」ボタンで編集中デッキをその場で対戦デッキに設定＝使用中は「★使用中」表示で押せない） | [Assets/Scripts/DeckBuilder/DeckBuilderPresenter.cs](Assets/Scripts/DeckBuilder/DeckBuilderPresenter.cs) |
...
| CPU デッキ構築 Presenter（相手スロット式・CpuRoster の各相手デッキを切り替えて編集。Editor 専用） | [Assets/Scripts/CpuDeckEditor/CpuDeckBuilderPresenter.cs](Assets/Scripts/CpuDeckEditor/CpuDeckBuilderPresenter.cs) |
...
| Home Presenter（CPU相手選択カルーセル・使用デッキ選択モーダル（各行の左にシンボルカード表示＝未設定はカード裏面）・左上のユーザーネーム表示＋鉛筆アイコンの編集モーダル（現在名プリフィル・空入力NG・16文字制限）・各モーダルの右上×ボタン含む） | [Assets/Scripts/Home/HomePresenter.cs](Assets/Scripts/Home/HomePresenter.cs) |
...
| 付与キーワード能力 enum（GrantKeyword の値2対応・守護/速攻/飛行/防人/強襲/デッキ攻撃×/射手） | [Assets/Scripts/Main/Card/GrantableKeyword.cs](Assets/Scripts/Main/Card/GrantableKeyword.cs) |
| カードID自動採番（属性×弾別・C{属性番号×1000+(弾-1)×100+連番}・弾1=オフセット0で既存ID維持・1属性1弾あたり99枚/最大9弾・Editor 専用） | [Assets/Scripts/Main/Card/CardIdAutoAssigner.cs](Assets/Scripts/Main/Card/CardIdAutoAssigner.cs) |
| カード編集ウィンドウ（検索・編集・追加・削除・並び替え（ID/コスト順）・弾フィルタ＋〈弾N〉表示・ID再採番・ゲーム使用/不使用切替・読み上げ話者の個別指定。新規追加は属性＋弾を選び、該当弾の SO が無ければ `Assets/Data/Set{弾}/{属性}/` に自動生成し CardDatabase へ自動登録。効果テキスト一括生成はメニュー `Card → 効果テキストを一括生成` から実行。Editor 専用） | [Assets/Scripts/Editor/CardEditorWindow.cs](Assets/Scripts/Editor/CardEditorWindow.cs) |
...
| カード詳細モーダル（キーワード能力アイコンをタップで効果説明を展開・任意のアクションボタンを下部に表示できる＝デッキ構築のシンボル設定で使用。右上の✕ボタンまたは外側クリックで閉じる＝閉じ判定は `PointerDownEvent`。閉じたとき `OnHidden` で通知＝チュートリアル「カードの見方」の誘導に使用） | [Assets/Scripts/Main/Card/CardDetailModal.cs](Assets/Scripts/Main/Card/CardDetailModal.cs) |
...
| 共通の勝利条件ルール（デッキ切れ＝オーバーリミット/勝利点20/キャラ8体） | [Assets/Scripts/Main/Game/WinRule.cs](Assets/Scripts/Main/Game/WinRule.cs) |
...
| Main Presenter（チュートリアル：誘導つきスクリプト対戦。固定デッキ・盤面プリセット・台本CPU・コーチ吹き出し・クリア/失敗演出） | [Assets/Scripts/Main/MainPresenter.Tutorial.cs](Assets/Scripts/Main/MainPresenter.Tutorial.cs) |
...
| Main Presenter（メインフェーズ・CPU選択・CPU難易度（中級以上は CostBoost／ダメージトリガー持ちを場に出さずコストに回す）・タップ/アンタップ・攻撃可否判定・行動予約＝解決アニメ中にドラッグした攻撃を `_queuedAttacks` に積み終わり次第順に実行、End で解決中にターン終了を予約。攻撃入力は `RefreshAttackInput` でフェーズ中常駐し解決中も生きる） | [Assets/Scripts/Main/MainPresenter.Phases.Main.cs](Assets/Scripts/Main/MainPresenter.Phases.Main.cs) |
...
| Main Presenter（自分の墓地からキャラをn体選んで場に出す。空きスロット数で上限・墓地順同期でインデックス解決） | [Assets/Scripts/Main/MainPresenter.Phases.Resolution.SummonFromGrave.cs](Assets/Scripts/Main/MainPresenter.Phases.Resolution.SummonFromGrave.cs) |
| Main Presenter（デッキから特徴指定でn枚手札に加える＋複数選択ピッカー `WaitForPlayerCardsPickAsync`＝墓地召喚と共通） | [Assets/Scripts/Main/MainPresenter.Phases.Resolution.AddToHandFromDeck.cs](Assets/Scripts/Main/MainPresenter.Phases.Resolution.AddToHandFromDeck.cs) |
...
| Main Presenter（ハンデス：相手が手札を捨てる。捨てるカードは持ち主が選ぶ＝ピッカー／CPUは低コスト順／オンラインは被害者→発動側へ捨てたID逆送信） | [Assets/Scripts/Main/MainPresenter.Phases.Resolution.Discard.cs](Assets/Scripts/Main/MainPresenter.Phases.Resolution.Discard.cs) |
...
| Main Presenter（カード飛行・ドロー・召喚登場演出・ダメージトリガー発動カットイン） | [Assets/Scripts/Main/MainPresenter.Animations.CardFly.cs](Assets/Scripts/Main/MainPresenter.Animations.CardFly.cs) |
...
| Main USS（盤面レイアウト／演出別に4分割。Overlays=全画面モーダル・FloatingLabels=効果ラベル/トースト・Highlights=選択/攻撃枠。Tutorial=チュートリアルのコーチ吹き出し） | [Assets/Scripts/Main/View/](Assets/Scripts/Main/View/) |
