...
| デッキ保存（9スロット制・各スロットにカード列/デッキ名/対戦に使う選択中スロットを PlayerPrefs 保存。旧単一デッキはスロット1へ自動移行） | [Assets/Scripts/Common/Deck/DeckRepository.cs](Assets/Scripts/Common/Deck/DeckRepository.cs) |
...
| デッキ構築 Presenter（9枠のスロット一覧↔編集の2画面・デッキ名リネーム。編集専用で対戦デッキ選択は Home 側） | [Assets/Scripts/DeckBuilder/DeckBuilderPresenter.cs](Assets/Scripts/DeckBuilder/DeckBuilderPresenter.cs) |
...
| Home Presenter（CPU相手選択カルーセル・使用デッキ選択モーダル・各モーダルの右上×ボタン含む） | [Assets/Scripts/Home/HomePresenter.cs](Assets/Scripts/Home/HomePresenter.cs) |
